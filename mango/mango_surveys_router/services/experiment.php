...
class Experiment {
	...
	function getExperiment($iExperimentId) {
		$sQuery = "SELECT id, name, login_phase, results_phase, generate_tokens, is_over FROM mango_experiment WHERE id = $iExperimentId";
		$oResult = $this->oDbConnection->query($sQuery);
		while($aRow = mysqli_fetch_array($oResult)) {
	 ...
	 * @return void
	 **/
	function saveExperiment($iExperimentId, $sExperimentName, $bExperimentLoginPhase, $bExperimentResultsPhase, $bExperimentGenerateTokens, $bExperimentIsOver, $aExperimentGames) {
		// If this experiment doesn't exist, add it
		if($iExperimentId == '') {
			$sQuery = "INSERT INTO mango_experiment (name, login_phase, results_phase, generate_tokens, is_over) VALUES ('$sExperimentName', $bExperimentLoginPhase, $bExperimentResultsPhase, $bExperimentGenerateTokens, $bExperimentIsOver)";
			$oResult = $this->oDbConnection->query($sQuery);
