			...
			function() {
				...
				$.ajax({
					...
					success: function(data) {
						...
						// From name, guess language.
						window.Mango.lang = (function(name) {
							var matches = name.match(/\[[A-Za-z]{2}\]/);
						...
						// Apply matching games. Remove duplicates of different languages.
						$('.list-games').slice(1).remove();
						$('.list-games option').first().attr('selected', 'selected');
						$('.list-games option').each(function(index, elm) {
							var $elm = $(elm);
							if ($elm.text().indexOf(window.Mango.lang) === -1) {
								$elm.remove();
							}
						});
						for(var i = 0; i < oExperiment.games.length; i++) {
							// Set the appropriate number of fields.
							if(!$('.list-games[index="' + oExperiment.games[i].survey_order + '"] select').length) {
								addNewGame(oExperiment.games[i].survey_order);
							}

							// Set the matching games
						...
						// Set experiment url. Modify language value in query.
						var COUNTRY_CODE_LENGTH = 2;
						var url = oExperiment.url;
						var lang = (function(l) {
							// e.g. window.Mango.lang is "[fr]", this returns "fr"
							if (!l) return null;
							return l.substr(1, COUNTRY_CODE_LENGTH);
						}(window.Mango && window.Mango.lang ? window.Mango.lang : null));

						if (lang) {
							var index = url.lastIndexOf('lang=') + 'lang='.length;
							url = url.slice(0, index) + lang + url.slice(index + COUNTRY_CODE_LENGTH);
						}

						$('.experiment-url').removeClass('hide');
						$('.experiment-url').attr('href', url);
					}
				});
			}
		);
